...
def get_gpt4_score(question, answer, ref_ans):
    prompt = {"name": "single-math-v1", "type": "single", "system_prompt": "You are a helpful assistant.", "prompt_template": "[Instruction]\nPlease act as an impartial judge and evaluate the quality of the response provided by an AI assistant to the user question displayed below. Your evaluation should consider correctness and helpfulness. You will be given a reference answer and the assistant's answer. Always remember that there are multiple ways to solve a math problem, and the score should be given mainly according to the correctness. If the answer is wrong, then you must give a low score. Begin your evaluation by comparing the assistant's answer with the reference answer. Identify and correct any mistakes. Be as objective as possible. After providing your explanation, you must rate the response on a scale of 1 to 10 by strictly following this format: \"[[rating]]\", for example: \"Rating: [[5]]\".\n\n[Question]\n{question}\n\n[The Start of Reference Answer]\n{ref_answer}\n[The End of Reference Answer]\n\n[The Start of Assistant's Answer]\n{answer}\n[The End of Assistant's Answer]", "description": "Prompt for general questions", "category": "math", "output_format": "[[rating]]"}
    ...
    score_res = gpt4_generate(judge_prompt, temp=0)
    return score_res
...
def gen_math_problem_template(question):
    prompt = 'This is a math question. Please solve the question step by step in a single paragraph and give the correct result in the second paragraph. Your answer should be in the following format: The answer is: XXX. XXX should be the most concise answer form, such as a single number without any explanation.\n\nQuestion: {question}\nYour output:'
...
def deep_search(task_name, seed_prompts):
    ...
    for idx in range(len(seed_prompts)):
        i = seed_prompts[idx]
        question_prompt = gen_math_problem_template(i['prompt'])
        ref_ans = optimize_func(question_prompt)
    ...
    while len(steps) < 15:
        ...
        optimized_prompt = """This task involves generating test cases for the math word task. Mathematic capability is an important topic in LLM research, where LLM needs to use several skills to complete a math problem.
...
Previous Prompts:\n\n"""
        ...
        optimized_prompt += """The objective is to create new prompts that are challenging for the language model, with a focus on diverse types of math word problems about "{task_name}". Each prompt should be solvable by a language model, complete, and aimed at achieving a lower score (indicating be difficult and complex).
...
5. Please focus on "{task_name}" problems. But don't express the task name explicitly in prompts.
...
Please generate a new test case. Output in json format, {"key_point": ..., "prompt": ...}"""
        ...
        try:
            ...
            question_prompt = gen_math_problem_template(new_prompt)
            ref_ans = optimize_func(question_prompt)
...
def gen_seed(task_name, categories):
    prompt_template = """Mathematic capability is an important topic in LLM research, where LLM needs to use several skills to complete a math problem. If possible, please generate test cases mainly on MATH WORD PROBLEMS in the current context, which represents real scenario combined with a math problem.
Here is a taxonomy for math word problems:
{categories}

Based on this, please generate 5 test case of "{task_name}" category to test if language models can solve math problems of "{task_name}". Key point is a short sentence summarizes the key point you want to test the language model. Besides, your test cases should cover different task types to increase prompt diversity. Please be as diverse as you can but focus on "{task_name}" and ensure the prompt is text-only (no multimodal). The answer of these test cases are expected not to be too long.
...
def analysis(task_names):
    prompt_template = """Mathematic capability is an important topic in LLM research, where LLM needs to use several skills to complete a math problem.
...
if __name__ == '__main__':
    with open('../data/math_cat.json', 'r') as f:
    ...
    main_task = args.task
    if main_cat in ['algebra', 'geometry']:
        points = list(categories['math'][main_cat][main_task].keys())
    else:
        points = list(categories['math'][main_task].keys())
        ...
        if idx == len(test_points) - 1:
            for x in range(3):
                ...
                if not judge_new_task(test_points, new_task):
                    ...
                    exit(0)
                if main_cat in ['algebra', 'geometry']:
                    categories['math'][main_cat][main_task][new_task] = {}
                else:
                    categories['math'][main_task][new_task] = {}
